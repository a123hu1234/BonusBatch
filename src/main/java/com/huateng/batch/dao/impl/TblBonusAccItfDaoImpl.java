 ...
 */
@Component
public class TblBonusAccItfDaoImpl implements TblBonusAccItfDao{

	@Autowired
	private JdbcTemplate template;
	@Override
	public int[] saveList(List<TblBonusAccItf> list) {
		int[] sRetun = template.execute("insert into " + TblBonusAccItf.getTableClum() +" values (" + TblBonusAccItf.getBeanClum() +")", new PreparedStatementCallback<int[]>() {

			@Override
			public int[] doInPreparedStatement(PreparedStatement ps) throws SQLException, DataAccessException {
				
					for(TblBonusAccItf inf : list) {
