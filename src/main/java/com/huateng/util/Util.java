...
public class Util {
	...
	/*
	 * public static boolean validateUserName(String username) { Pattern p =
	 * Pattern.compile("^\\w+$"); Matcher m = p.matcher(username); if (m.find())
	...
	/*
	 * public static String replaceComma(String text) { if (text != null) { text
	 * = text.replaceAll("，", ","); } return text; }
	 * 
	 * public static String replaceBr(String text) { if (text != null) { text =
	 * text.replaceAll("\n", "<BR>"); } return text; }
	 */
	...
	public static String getFormatYYMMDDHHMMSS(String strtm) {
		...
		if (!start_time.trim().equals("")) {
			...
			start_time = (str_hour + ":" + str_minute + ":" + str_second).substring(0, 8);
		}
		return (str_year + "-" + str_month + "-" + str_date + " " + start_time).trim();
	}
	...
	/*
	 * obj 奖金额除与被除数 保留两个小数 四舍5入
	 * by ygq
	 */
	public static String tradAmt(Object obj, Object ob) {
		BigDecimal bonus = new BigDecimal(Double.valueOf((String) obj));
		BigDecimal b = new BigDecimal(Double.valueOf((String) ob));

		return bonus.multiply(b).setScale(2, BigDecimal.ROUND_HALF_UP) + "";
	}
	...
	/*  param s  ascii 字符串表达式 如 0x03
	 * 将字符串的ascii转换层String
	 * by ygq
	 */
	public static String asciiToStrings(String s) throws UnsupportedEncodingException {
		byte[] tBytes = hexStringToBytes(s);
		String str = new String(tBytes, "ascii");
		return str;
	}
	/*
	 * 将字符串的ascii转换层btye[]
	...
	public static String  utf8StrtoGbk(String str) throws UnsupportedEncodingException{
